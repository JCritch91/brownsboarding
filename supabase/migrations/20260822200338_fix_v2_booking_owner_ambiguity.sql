...
begin
  ...
  if p_dog_ids is null
    or cardinality(p_dog_ids) = 0 then
  ...
  select count(distinct selected_dog.dog_id)
  into distinct_dog_count
  from unnest(p_dog_ids)
    as selected_dog(dog_id);
  ...
  /*
   * Qualify every dogs column to prevent collision with
   * the owner_id output parameter declared by RETURNS TABLE.
   */
  select count(*)
  into active_owner_dog_count
  from public.dogs as selected_dog
  where selected_dog.id = any(p_dog_ids)
    and selected_dog.owner_id = p_owner_id
    and selected_dog.active = true;
  ...
  if p_booking_type = 'boarding' then
    if p_daycare_session is not null then
      raise exception
        'BOARDING_DAYCARE_SESSION_NOT_ALLOWED';
    end if;
...
comment on function
  public.create_pending_booking_v2_atomic(
    uuid,
    uuid[],
    public.booking_type,
    public.daycare_session_type,
...
is
  'Atomically creates a Pending Booking Engine V2 booking and links one or two active dogs belonging to the booking owner.';