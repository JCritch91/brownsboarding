  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "You do not have permission to manage Meet & Greet approval.",
        },
        {
          status: 403,
        }
    ...
    const body =
      (await request.json()) as UpdateMeetAndGreetRequest;

    if (typeof body.approved !== "boolean") {
      return NextResponse.json(
        {
          error:
            "The requested Meet & Greet approval status is invalid.",
        },
        {
          status: 400,
        }
    ...
    const existingApprovalStatus =
      customer.meet_and_greet_approved === true;
    ...
    const meetAndGreetApproved =
      updatedCustomer.meet_and_greet_approved ===
      true;
    ...
    console.error(
      "Admin Meet & Greet approval update failed:",
      error
    );
